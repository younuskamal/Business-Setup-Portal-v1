...
const Navbar: React.FC = () => {
  ...
  return (
    <>
      <header
        className={`fixed top-0 left-0 right-0 w-full font-sans z-50 transition-all duration-500 ease-in-out ${isScrolled
        ...
        <div className={`hidden lg:block transition-all duration-500 ease-in-out border-b border-white/10 overflow-hidden ${isScrolled ? 'max-h-0 opacity-0' : 'max-h-12 opacity-100'}`}>
          <div className="container mx-auto px-4 lg:px-8 flex justify-between items-center pb-3 text-white/80 text-xs font-medium tracking-wide">
            ...
            <div className="flex items-center gap-6">
              <a href="tel:+905527324029" className="flex items-center gap-2 hover:text-white transition-colors group" dir="ltr">
                <Phone size={14} className="group-hover:scale-110 transition-transform" />
                <span>+90 552 732 40 29</span>
              </a>
        ...
        <nav className="w-full relative z-50">
              ...
              <div className="hidden lg:flex items-center gap-8">
                {navItems.map((item) => (
                  <NavLink
                    ...
                    to={item.path}
                    className={({ isActive }) =>
                      `text-[15px] font-bold transition-all duration-300 relative py-2 ${isActive
                        ? 'text-white'
          ...
          */}
          <div
            ...
            className={`absolute top-full left-0 w-full bg-white shadow-2xl border-t border-gray-100 transition-all duration-300 origin-top transform ${isSearchOpen
      ...
      < div
        className={`lg:hidden fixed inset-0 z-[100] transition-opacity duration-300 ${isOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`
        }
      >
        {/* Backdrop */}
        < div
        ...
        {/* Drawer */}
        < div
          className={`absolute top-0 bottom-0 w-[85%] max-w-[360px] bg-[#0B2B5B] shadow-2xl transform transition-transform duration-500 cubic-bezier(0.16, 1, 0.3, 1) flex flex-col h-full ${isOpen ? 'ltr:translate-x-0 rtl:translate-x-0' : 'ltr:-translate-x-full rtl:translate-x-full'} ${language === 'ar' ? 'right-0' : 'left-0'}`}
            ...
            <div className="space-y-2">
              {navItems.map((item, idx) => (
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `block px-5 py-4 rounded-xl text-lg font-bold transition-all border border-transparent ${isActive
